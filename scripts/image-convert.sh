...
get_srcset()
{
  ...
  for image_identify in "${image_identify_array[@]}"; do
    ...
    if [[ "${image_name}" == *${2} ]]; then
      ...
      image_url="${web_image_dir}/${image_name}"
      image_with_width="${image_url} ${dimensions[0]}w"
      if [ "$srcset" = "$initial_srcset" ]; then
        srcset="${srcset}${image_with_width}"
      else
        srcset="${srcset}, ${image_with_width}"
      fi
    fi
